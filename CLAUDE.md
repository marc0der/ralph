...
Review audits build fidelity only. It never judges the plan: `plan` converged on that file and already settled what the specs require and how to decompose it, so review's one question is whether `build` honoured the item. It makes two checks per `- [x]` item — did `build` implement what the item names, and is the code it wrote defective — and those two checks are the two severity levels, `Critical` winning any finding where both fit. Check 2 covers every change in the commits that shipped the item, not just the item's `Files`, because `prompts/build.md` lets `build` fix an unrelated red suite outside the `Scope`. Review never reads `specs/`; a finding it cannot attribute to a shipped item goes in its final message and no file.

Review edits the plan under the plan-phase rules, plus three of its own. It may refine, reorder and supersede open items, but it never alters a `- [x]` marker: a shipped item that fails its claim becomes a new item naming the defect, because un-ticking hides that a defect escaped and lets an item oscillate between `[ ]` and `[x]` across review and build runs. Every supersession review makes is recorded in `PROGRESS.md`, because the next plan run resolves a `[~]` item by reading that entry and will otherwise resurrect the item as open. Review never reads, creates or edits anything under `specs/` — reading it would let review re-litigate planning, and writing it would both manufacture its own standard and defeat convergence, since `plan_state_hash` fingerprints `specs/` too.

A review pass files **at most 5 open findings**. The cap is a standing limit on `IMPLEMENTATION_PLAN.md`, not a per-pass quota, so a pass that finds the queue already full changes nothing and the loop converges on it. One context audits the whole plan: the judgement is never split across subagents, because a subagent holding a slice of the items cannot rank its findings against the rest, and slices sum instead of competing. Every finding traces to one shipped item and proves `build` fell short of it — a test the item never called for, wording in a document no item named, a naming or style preference, and an item the reviewer would have planned differently are unfileable at any severity. A test or a document the item *did* name is fileable and is `Critical`: the item asked for a named deliverable and it is absent, which is an unmet step like any other. A finding's `Spec:` field names the audited item as `IMPLEMENTATION_PLAN.md` plus its quoted title, never a position and never the audited item's own citation.
